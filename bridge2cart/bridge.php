...
class M1_Bridge_Action_CreateRefund
{
  ...
  private function _checkRequestKey($requestKey)
  {
    ...
    curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(array('request_key' => $requestKey, 'store_key' => M1_TOKEN)));
  ...
  public function perform(M1_Bridge $bridge)
  {
    ...
    $refundMessage = $_POST['refund_message'];
    $itemsData = json_decode($_POST['items'], true);
    $totalRefund = isset($_POST['total_refund']) ? $_POST['total_refund'] : null;
    $restockItems = $_POST['restock_items'];
      ...
      switch ($bridge->config->cartType) {
        ...
        case 'Magento1212':
          ...
          if (version_compare($version, '2.0.0', '<')) {
            ...
            $creditmemoRepository = $objectManager->create('\Magento\Sales\Model\Order\CreditmemoRepository');
            ...
            foreach ($invoices as $invoice) {
              ...
              if ($restockItems) {
                foreach ($creditmemo->getAllItems() as $key => $item) {
                  $stockItem = $stockItemRepository->get($item->getProductId());
                  if ($stockItem && $stockItem->getId() && $stockItem->getManageStock()) {
                    $qty = $stockItem->getQty() + $item->getQty();
                    $stockItem->setQty($qty);
                    $stockItem->setIsInStock((int)($qty > 0));
        ...
        case 'Wordpress':
          ...
          if ($bridge->config->cartId === 'Woocommerce') {
            ...
            foreach ($itemsData as $itemData) {
              $lineItems[$itemData['order_product_id']]['qty'] = isset($itemData['quantity']) ? $itemData['quantity'] : 0;
              $lineItems[$itemData['order_product_id']]['refund_total'] = isset($itemData['total_price']) ? $itemData['total_price'] : 0;
            }
  ...
  public function __construct()
  {
    ...
    if (isset($config)) {
      ...
      if (isset($constants['DB_NAME'])
          && isset($constants['DB_USER'])
          && isset($constants['DB_PASSWORD'])
          && isset($constants['DB_HOST'])
          && isset($tblPrefixMatch[1])
      ) {
...
{
  public $config = null; // config adapter
  public $noResult = array('delete', 'update', 'move', 'truncate', 'insert', 'set', 'create', 'drop', 'replace');
  public $dataBaseHandle = null;
...
define('M1_BRIDGE_VERSION', '63');
define('M1_BRIDGE_DOWNLOAD_LINK', 'https://api.eu.api2cart.com/v1.0/bridge.download.file?update&whitelabel=true');
define('M1_BRIDGE_CHECK_REQUEST_KEY_LINK', 'https://app.eu.api2cart.com/request/key/check');
